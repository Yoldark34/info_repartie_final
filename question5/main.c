...
void trtSIGUSR1 (int signalRecu, siginfo_t* info, void* pasUtileIci)
 {
  printf("Pere       : Le processus numéro %d à gagné l'élection\n", last_pid); 
  fflush(stdout);
kill (pidFils, SIGTERM);
kill (pidPetitFils, SIGTERM);
kill (pidPere, SIGTERM);
 }

void trtSIGTERM (int signalRecu, siginfo_t* info, void* pasUtileIci)
 {
  printf("Le processus numero %d receptionne le signal %d (SIGTERM) envoye par le processus %d et s'arrete \n", getpid(), signalRecu, info->si_pid); 
  fflush(stdout);
  exit (4);
 }
...
int main (int argc, char** argv)
{
  ...
  struct sigaction strSIGTERM;
                   strSIGTERM.sa_sigaction=&trtSIGTERM;
                   strSIGTERM.sa_flags=SA_SIGINFO;

  sigemptyset(&strSIGTERM.sa_mask);
  sigaction(SIGTERM,&strSIGTERM, NULL);
